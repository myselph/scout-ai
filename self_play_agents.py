...
class TransformerAgentCollection(AgentCollection):
    @staticmethod
    def create_agents(num_agents: int, device: torch.device = torch.de'simple_agent_0_it_79_skill_1.95.pth'vice("cpu")) -> list[Agent]:
        embed_dim = 64
        num_heads = 4
        num_layers = 4
        dim_ffd = 32
        ...
        return [
            TransformerAgent(
                embed_dim,
                num_heads,
                dim_ffd,'simple_agent_0_it_79_skill_1.95.pth'
                num_layers,
                policy_lr,
                value_fn,
                value_optim,
